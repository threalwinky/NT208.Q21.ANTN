...
Studify là nền tảng web full-stack dành cho sinh viên Trường Đại học Công nghệ Thông tin - ĐHQG-HCM. Bản `1.0.5` tập trung vào trải nghiệm chat thời gian thực: MiMo chạy qua Anthropic-compatible SSE, RAG tiếng Việt được tăng cường bằng structured facts, web search riêng bằng Selenium + Chromium, và giao diện chat có hai chế độ **Nhanh** / **Mở rộng**.

## 1. Cấu trúc phiên bản

Trong thư mục cha `1.0.5/`:
...
Nginx route `/` về frontend và route `/api/v1/*`, `/health`, `/docs`, `/openapi.json` về backend. Do backend trong bản local đang chạy `network_mode: host`, Nginx tự nhận Docker gateway khi khởi động; nếu môi trường deploy cần ép IP cụ thể, set `NGINX_BACKEND_GATEWAY`.
...
local = Path('1.0.5/local')
deploy = Path('1.0.5/deploy')
