      ...
      <div className="grid gap-4 xl:grid-cols-[0.8fr_1.2fr]">
        ...
        <AppCard title="Kết nối demo" subtitle="Nút này chỉ lưu trạng thái preview để trình bày flow tích hợp, không lưu token nhạy cảm.">
          <label className="block">
            ...
            <input value={displayName} onChange={(event) => setDisplayName(event.target.value)} className="mt-2 w-full rounded-[18px] border border-[color:var(--line)] bg-[color:var(--surface-soft)] px-4 py-3 text-sm outline-none" />
          </label>
          <div className="mt-4 flex flex-wrap gap-3">
            <button type="button" onClick={async () => setStatus(await connectSpotifyPreview({ display_name: displayName, scopes: ["user-read-email"] }))} className="btn-primary rounded-2xl px-5 py-3 text-sm font-semibold transition">Kết nối preview</button>
            <button type="button" onClick={async () => setStatus(await disconnectSpotify())} className="btn-secondary rounded-2xl px-5 py-3 text-sm font-semibold transition">Ngắt kết nối</button>
          </div>
        </AppCard>
      </div>
    </AppShell>
