...
import { Globe, MessageCircleHeart, Plus, Send, Trash2 } from "lucide-react";
import { useEffect, useRef, useState } from "react";

import { AppShell } from "@/components/app-shell";
import { MarkdownContent } from "@/components/markdown-content";
import { deleteChatSession, getChatSessions, streamChatMessage, type ChatMode, type ChatReply } from "@/lib/api";
              ...
              return sortSessions([
                {
                  ...
                  mode: chatMode === "extended" ? "STUDIFY_EXTENDED" : "STUDIFY_QUICK",
            ...
            {!hasConversation ? (
              <div className="flex flex-1 flex-col items-center justify-center">
                <div className="w-full max-w-3xl text-center">
                  ...
                  <div className="mt-8">
                    <ChatComposer content={content} loading={loading} chatMode={chatMode} onChange={setContent} onSubmit={() => void handleSend()} onChatModeChange={setChatMode} />
                  </div>
              ...
              <div className="space-y-10">
                ...
                {streamingReply && streamingReply.answer.trim() ? (
                  <div className="flex justify-start">
                    <div className="w-full max-w-3xl">
                      <AssistantStatusHeader status={assistantStatus || "Studify đang trả lời..."} />
                      <div className="rounded-[26px] border border-[color:var(--line)] bg-[color:var(--surface-soft)] px-5 py-4">
                        <MarkdownContent content={streamingReply.answer} className="text-sm" />
                        <span className="inline-block w-0.5 h-4 bg-current align-middle ml-0.5 animate-pulse opacity-60" />
                      </div>
                    </div>
                  </div>
                ) : null}
