...
class StructuredFactType(str, Enum):
...
class CollectedDocument(Base, TimestampMixin):
    ...
    facts: Mapped[list["StructuredKnowledgeFact"]] = relationship(back_populates="document", cascade="all, delete-orphan")
...
class StructuredKnowledgeFact(Base, TimestampMixin):
    __tablename__ = "structured_knowledge_facts"

    id: Mapped[int] = mapped_column(primary_key=True)
    document_id: Mapped[int] = mapped_column(ForeignKey("collected_documents.id", ondelete="CASCADE"), nullable=False, index=True)
    fact_type: Mapped[StructuredFactType] = mapped_column(String(60), nullable=False, index=True)
    title: Mapped[str] = mapped_column(String(255), nullable=False)
    fact_text: Mapped[str] = mapped_column(Text, nullable=False)
    school_year: Mapped[str | None] = mapped_column(String(20), nullable=True, index=True)
    effective_from: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    effective_to: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    applies_to_programs: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)
    applies_to_cohorts: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)
    value_json: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    confidence_level: Mapped[ConfidenceLevel] = mapped_column(String(20), default=ConfidenceLevel.HIGH, nullable=False)

    document: Mapped["CollectedDocument"] = relationship(back_populates="facts")
