...
class ChatService:
    ...
    def _direct_answer_rules(self, effective_query: str) -> str:
        ...
        rules = [
            ...
            "Với câu hỏi học vụ, trình bày đúng các mục: ### Trả lời nhanh, ### Chi tiết, ### Bạn nên làm gì tiếp theo?, ### Nguồn tham khảo.",
    ...
    def _context_limit(self, analysis: QueryAnalysis, effective_query: str) -> int:
        topics = self._query_topics(effective_query)
        if "leadership" in topics:
            return 4
        if analysis.category == "ACADEMIC":
            return 8 if topics else 6
        if analysis.category == "ANNOUNCEMENT":
            return 6 if topics.intersection({"schedule", "scholarship", "tuition", "graduation", "registration", "annual_plan", "academic_warning"}) else 4
        return 4
    ...
    async def _prepare_turn(
        ...
        contexts = await self.rag.retrieve(db, effective_query, limit=self._context_limit(analysis, effective_query))
    ...
    async def answer(self, db: Session, session: ChatSession, content: str) -> ChatReply:
        ...
        try:
            answer = await self.llm.chat(prepared.messages)
        except Exception as exc:
            print(f"[chat] llm chat failed: {exc}")
    ...
    async def stream_answer(self, db: Session, session: ChatSession, content: str) -> AsyncIterator[dict]:
        ...
        try:
            async for chunk in self.llm.stream_chat(prepared.messages):
