    ...
    <AppShell pageTitle="Danh mục môn học" pageDescription="Tra cứu học phần, tín chỉ, nhóm yêu cầu và tiên quyết trong CTĐT demo.">
      <AppCard title="Tìm môn học" subtitle="Có thể tìm bằng mã môn hoặc tên môn.">
        <div className="flex flex-col gap-3 md:flex-row">
          ...
          />
          <button type="button" onClick={() => void load()} className="btn-primary rounded-2xl px-5 py-3 text-sm font-semibold transition">
            Tìm kiếm
          </button>
        </div>
      </AppCard>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {courses.map((course) => (
          <div key={course.id} className="rounded-[24px] border border-[color:var(--line)] bg-[color:var(--surface)] p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
