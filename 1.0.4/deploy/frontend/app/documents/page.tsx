    ...
    <AppShell pageTitle="Tài liệu tri thức" pageDescription="Duyệt tài liệu đã crawl/tải lên và kiểm nhanh độ chính thức của nguồn.">
      <AppCard title="Lọc tài liệu" subtitle="Tìm trong title, summary và nội dung đã làm sạch.">
        <div className="flex flex-col gap-3 md:flex-row">
          <input value={query} onChange={(event) => setQuery(event.target.value)} className="min-h-12 flex-1 rounded-[18px] border border-[color:var(--line)] bg-[color:var(--surface-soft)] px-4 text-sm outline-none" placeholder="Tìm học phí, tốt nghiệp, học bổng..." />
          <button type="button" onClick={() => void load()} className="btn-primary rounded-2xl px-5 py-3 text-sm font-semibold transition">Tìm tài liệu</button>
        </div>
      </AppCard>
      <div className="grid gap-4 xl:grid-cols-2">
        {documents.map((item) => (
          <a key={String(item.id)} href={String(item.url)} target="_blank" rel="noreferrer" className="rounded-[24px] border border-[color:var(--line)] bg-[color:var(--surface)] p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)] transition hover:border-[color:var(--accent)]/30">
