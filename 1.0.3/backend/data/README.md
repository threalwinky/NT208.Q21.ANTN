...
- `evaluation/uit_rag_suite.json`
  - bộ benchmark để đo retrieval, fact hit và domain hit của RAG
...
- mỗi record khi import sẽ được gắn freshness metadata và sinh structured facts
...
Importer hỗ trợ resume:

```bash
cd backend
python -m app.scripts.import_snapshot_data --progress-file data/processed/import_progress.json
python -m app.scripts.import_snapshot_data --reset-progress
python -m app.scripts.import_snapshot_data --limit 50
```
...
```bash
cd backend
python -m app.scripts.evaluate_rag --benchmark-file data/evaluation/uit_rag_suite.json
```
