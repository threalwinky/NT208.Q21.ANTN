...
def seed_demo_environment(db):
    vu_user, vu_profile = create_student(db, "24522045", "Vo Quang Vu", "Khoa Mạng máy tính và Truyền thông", "An toàn thông tin", "ATTN2024")
...
def test_degree_audit_identifies_missing_core_courses() -> None:
    ...
    try:
        ...
        assert audit.program_name == "Cử nhân An toàn thông tin"
        assert audit.completion_percent > 0
        assert "IT002" in audit.missing_core_courses
        assert any(item.code == "NT301" for item in audit.required_courses)
    finally:
        db.close()
...
def test_semester_planning_prioritizes_failed_or_blocking_courses() -> None:
    ...
    try:
        ...
        planning = service.build_semester_planning(db, tu_user)
...
def test_academic_risk_alert_becomes_high_for_weaker_profile() -> None:
    ...
    try:
        ...
        risk = service.build_academic_risk(db, vu_user)
