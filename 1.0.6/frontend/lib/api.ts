...
export type Announcement = Record<string, unknown> & {
  ...
  isOfficialUit?: boolean;
};

export type StudyDocument = Record<string, unknown> & {
  id: number;
  title: string;
  url: string;
  summary?: string | null;
...
export type AcademicEvent = Record<string, unknown> & {
  id: number;
  title: string;
  group_name?: string | null;
...
export type ClassScheduleItem = Record<string, unknown> & {
  id: number;
  course_code: string;
  course_name: string;
...
export type ExamScheduleItem = Record<string, unknown> & {
  id: number;
  course_code: string;
  course_name: string;
  room_name?: string | null;
...
export type ChatMode = "quick" | "extended";
...
export async function logout() {
  return request<{ logged_out: boolean }>("/auth/logout", { method: "POST" }, { logged_out: true });
}
...
export async function getAnnouncements(groupName?: string) {
  ...
  return request<Announcement[]>(`/announcements${query}`, undefined, []);
}
...
export async function getStudyDocuments() {
  return request<StudyDocument[]>("/planner/documents", undefined, []);
}

export async function getDocuments(q?: string) {
  const query = q ? `?q=${encodeURIComponent(q)}` : "";
  return request<StudyDocument[]>(`/documents${query}`, undefined, []);
}
...
export async function getAcademicEvents() {
  return request<AcademicEvent[]>("/planner/events", undefined, []);
}

export async function getClassSchedule() {
  return request<ClassScheduleItem[]>("/planner/class-schedule", undefined, []);
}

export async function getExamSchedule() {
  return request<ExamScheduleItem[]>("/planner/exam-schedule", undefined, []);
}

export async function getTasks() {
  return request<Task[]>("/planner/tasks", undefined, []);
}

export async function createTask(payload: Record<string, unknown>) {
  return request<Task>("/planner/tasks", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
...
export async function completeTask(taskId: number) {
  return request<Task>(`/planner/tasks/${taskId}/complete`, { method: "PATCH" });
}
...
export async function sendChatMessage(payload: { session_id?: number | null; content: string; chat_mode?: ChatMode }) {
...
export async function streamChatMessage(
  payload: { session_id?: number | null; content: string; chat_mode?: ChatMode },
  handlers?: {
...
// ── Ôn tập (Notebook RAG từ PDF cá nhân) ──────────────────────────────────────
...
export type NotebookDetail = Notebook & { documents: RevisionDoc[] };
export type RevisionCitation = { doc_title: string; excerpt: string };
export type RevisionAnswer = { answer: string; citations: RevisionCitation[] };

export async function getNotebooks() {
  return request<Notebook[]>("/revision/notebooks", undefined, []);
}

export async function createNotebook(title: string) {
  return request<Notebook>("/revision/notebooks", { method: "POST", body: JSON.stringify({ title }) });
}

export async function getNotebook(id: number) {
  return request<NotebookDetail>(`/revision/notebooks/${id}`);
}

export async function deleteNotebook(id: number) {
  return request<{ deleted: boolean }>(`/revision/notebooks/${id}`, { method: "DELETE" });
}

export async function uploadNotebookDocument(notebookId: number, file: File, title?: string) {
  const form = new FormData();
  form.append("file", file);
  if (title) form.append("title", title);
  return requestForm<RevisionDoc>(`/revision/notebooks/${notebookId}/documents`, form);
}

export async function deleteNotebookDocument(notebookId: number, documentId: number) {
  return request<{ deleted: boolean }>(`/revision/notebooks/${notebookId}/documents/${documentId}`, {
    method: "DELETE",
  });
}

export async function askNotebook(notebookId: number, question: string) {
  return request<RevisionAnswer>(`/revision/notebooks/${notebookId}/ask`, {
    method: "POST",
