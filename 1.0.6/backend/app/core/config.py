...
class Settings(BaseSettings):
    ...
    # LLM provider mặc định: gpt (OpenAI-compatible router GPT-5.5).
    # Vẫn hỗ trợ "mimo" (MiniMax Anthropic API) và "ollama" để backward compat.
    llm_provider: str = "gpt"

    # ── GPT (OpenAI Chat Completions-compatible) ───────────────────────────────
    # URL + key KHÔNG hardcode trong code; cấu hình qua .env (GPT_BASE_URL, GPT_API_KEY).
    gpt_base_url: str = ""
    gpt_api_key: str = ""
    gpt_chat_model: str = "cx/gpt-5.5"
    gpt_timeout_seconds: int = 120
    # Để None thì KHÔNG gửi (an toàn với reasoning model vốn chặn các tham số này).
    gpt_temperature: float | None = None
    gpt_max_tokens: int | None = None

    # ── MiMo (MiniMax) – giữ lại để backward compat ─────────────────────────────
    ...
    crawler_user_agent: str = "StudifyBot/1.0.6 (+https://uit.edu.vn)"
    ...
    # Streaming bật mặc định từ v1.0.6 (Anthropic SSE realtime)
...
def validate_runtime_config(settings: Settings | None = None) -> None:
    ...
    if current.llm_provider not in {"gpt", "mimo", "ollama"}:
        raise RuntimeError("LLM_PROVIDER chỉ hỗ trợ gpt, mimo hoặc ollama.")
    if current.embedding_provider != "ollama":
        raise RuntimeError("EMBEDDING_PROVIDER hiện chỉ hỗ trợ ollama.")
    if current.llm_provider == "gpt" and _is_placeholder(current.gpt_api_key) and current.is_production:
        raise RuntimeError("GPT_API_KEY bắt buộc khi APP_ENV=production và LLM_PROVIDER=gpt.")
