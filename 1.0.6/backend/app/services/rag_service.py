...
class RagService:
    ...
    def _extract_tokens(self, query: str) -> list[str]:
        ...
        if "leadership" in topics:
            tokens.extend(["ban", "giam", "hieu", "truong", "pho", "hieu", "truong"])
    ...
    def _source_hint_score(self, query: str, document: CollectedDocument) -> float:
        ...
        if "leadership" in topics and self._has_any(
            haystack,
            ["ban giam hieu", "hieu truong", "pho hieu truong", "pho hieu truong phu trach", "nguyen tan tran minh khang", "nguyen luu thuy ngan"],
        ):
            score += 0.8
    ...
    def _freshness_adjustment(self, query: str, document: CollectedDocument) -> tuple[float, float]:
        normalized_query = self._normalize(query)
        asks_recent = self._has_any(normalized_query, ["moi nhat", "hien tai", "nam nay", "cap nhat", "2026", "2025", "2024"])
        updated_at = document.updated_source_at or document.published_at
        if not updated_at:
            return 0.0, 0.08 if asks_recent else 0.0
        if updated_at.tzinfo is None:
            updated_at = updated_at.replace(tzinfo=timezone.utc)
        age_days = max(0, (datetime.now(timezone.utc) - updated_at).days)
        if age_days <= 180:
            return (0.12 if asks_recent else 0.06), 0.0
        if age_days <= 730:
            return (0.05 if asks_recent else 0.02), 0.0
        return 0.0, 0.12 if asks_recent else 0.04
    ...
    async def retrieve(self, db: Session, query: str, limit: int = 4) -> list[RetrievedContext]:
        ...
        for candidate in candidates.values():
            ...
            candidate.freshness_bonus, candidate.stale_penalty = self._freshness_adjustment(query, candidate.document)
            official_source_bonus = 0.12 if candidate.document.is_official_uit else 0.0
            final_score = (
                (candidate.vector_score * 0.35)
                + (candidate.lexical_score * 0.5)
                + candidate.source_score
                + official_source_bonus
                + candidate.freshness_bonus
                - candidate.stale_penalty
            )
            if candidate.vector_score == 0 and candidate.lexical_score == 0 and candidate.source_score <= 0:
                continue
        ...
        if ranked_contexts and ranked_contexts[0].score < 0.28:
            return []
