...
from app.models.academic import ClassSchedule, ExamSchedule, Task, TaskStatus, normalize_task_status
...
async def overview(db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> DashboardOverview:
    ...
    advisor_service = AcademicAdvisorService()
    announcements = (
    ...
    try:
        advisor_summary = advisor_service.dashboard_summary(db, user)
    except Exception:
        advisor_summary = {
            "completionPercent": 0,
            "remainingCredits": 0,
    ...
    return DashboardOverview(
        ...
        upcoming_tasks=[
            DashboardTask(
                ...
                due_at=item.due_at,
                status=normalize_task_status(item.status),
        ...
        metrics={
            ...
            "openTasks": db.query(Task).filter(Task.user_id == user.id, Task.status != TaskStatus.DONE.value).count(),
            "upcomingExams": upcoming_exam_count,
